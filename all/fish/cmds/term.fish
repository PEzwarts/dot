...
function t
    if [ "$(uname)" = Linux ]
        if [ (hyprctl -j activeworkspace | jq -r '.windows') = 0 ]
            neovide -- -c ":lua vim.g.term = false vim.g.neovide_padding_left = 500 vim.g.neovide_padding_right = 500"
        else if [ (hyprctl -j activeworkspace | jq -r '.windows') > 1 ]
            neovide -- -c ":lua vim.g.term = false vim.g.neovide_padding_left = 0 vim.g.neovide_padding_right = 0"
        end
